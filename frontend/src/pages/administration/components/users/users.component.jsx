...
export default function Users(props) {
    ...
    const columns = [
        {
          field: 'name', headerName: strings.adminstration.users.list.name, flex: 1
        },
        {
          field: 'email', headerName: strings.adminstration.users.list.email, flex: 1
        },
        {
          field: 'role', headerName: strings.adminstration.users.list.role, flex: 1,  valueGetter: (params) => params.row.role === ROLETYPE.ADMIN ? strings.roles.admin : strings.roles.user
        },
        {
          field: 'divisions', headerName: strings.adminstration.users.list.access, flex: 1,  valueGetter: (params) => params.row.divisions.filter((division) => division.type !== DIVISION_TYPE.common_area).map((division) => { return division.name }).join(', ')
        },
        {
          field: "action",
          ...
          renderCell: (params) => {
            ...
            return (
              <IconButton onClick={onClick} color="primary" aria-label="see user details" component="label">
                <ReadMoreIcon />
              </IconButton>
            )
            ;
            ...
            <h1 className="content-title">{strings.adminstration.users.title}</h1>
                <div className="users-data-table-container">
                    <DataGrid
                        ...
                        isRowSelectable={(params) => params.row.role !== ROLETYPE.ADMIN }
                    />
                </div>
            <Zoom
                in={true}
